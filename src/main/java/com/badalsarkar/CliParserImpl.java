...
public class CliParserImpl implements CliParser {
	...
	public void registerCliOption(CliOption option) {
		OPTIONS.addOption(Option.builder(option.getShortName()).longOpt(option.getLongName())
				.desc(option.getDescription()).build());

	}

	@Override
	public void defineCliOptionWithArgument(CliOption option) {
		OPTIONS.addOption(Option.builder(option.getShortName()).longOpt(option.getLongName()).hasArg()
				.desc(option.getDescription()).build());
	...
	public void defineCliGroupOption(CliOption... options) {
		...
		for (CliOption c : options) {
			groupOptions.addOption(
					Option.builder(c.getShortName()).longOpt(c.getLongName()).desc(c.getDescription()).build());
		}
